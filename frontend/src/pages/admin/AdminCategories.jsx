...
import { TagIcon, PlusIcon, Edit3Icon, Trash2Icon, XIcon } from "../../components/Icons";
...
export default function AdminCategories() {
  ...
  const loadCategories = async () => {
    setLoading(true);
    try {
      const data = await categoryService.getAllWithCounts();
      setCategories(data);
    } catch (err) {
      console.error(err);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory) {
        ...
        addToast(`Category "${formData.name}" added!`, "success");
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      addToast(`Category "${deleteCat.name}" removed.`, "info");
      setIsDeleteModalOpen(false);
      setDeleteCat(null);
      await loadCategories();
    } catch (err) {
      const msg = err.response?.data?.message || "Cannot delete category with associated books.";
      addToast(msg, "error");
      setIsDeleteModalOpen(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Category Classification</h1>
          <p className="text-slate-300 text-sm mt-1">Organize books into genres and academic departments.</p>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="bg-slate-900/90 rounded-3xl p-12 border border-slate-800 text-center max-w-md mx-auto shadow-xl">
          ...
          <p className="text-xs text-slate-400 mt-1">Create genres to classify your collection.</p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-sm text-slate-300">
              <thead className="bg-slate-950/80 text-slate-400 text-xs font-bold uppercase tracking-wider border-b border-slate-800">
                <tr>
                  ...
                  <th className="px-6 py-4">Catalog Books</th>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Overview of this genre or branch..."
                  className="w-full px-3.5 py-2.5 rounded-xl border border-slate-800 bg-slate-950 text-white placeholder-slate-500 text-xs focus:outline-none focus:ring-2 focus:ring-indigo-500"
                ></textarea>
              </div>
      ...
      <ConfirmModal
        ...
        title="Delete Category"
        message={`Delete "${deleteCat?.name}"? Note that categories with active books cannot be deleted.`}
